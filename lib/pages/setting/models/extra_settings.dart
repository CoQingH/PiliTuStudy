...
import 'package:pilistudy/common/widgets/image/custom_grid_view.dart'
    show ImageModel;
import 'package:pilistudy/common/widgets/pendant_avatar.dart';
...
import 'package:pilistudy/models/common/audio_normalization.dart';
import 'package:pilistudy/models/common/dynamic/dynamics_type.dart';
import 'package:pilistudy/models/common/member/tab_type.dart';
import 'package:pilistudy/models/common/reply/reply_sort_type.dart';
...
import 'package:pilistudy/models/dynamics/result.dart';
import 'package:pilistudy/pages/common/slide/common_slide_page.dart';
import 'package:pilistudy/pages/home/controller.dart';
import 'package:pilistudy/pages/hot/controller.dart';
import 'package:pilistudy/pages/main/controller.dart';
import 'package:pilistudy/pages/setting/models/model.dart';
import 'package:pilistudy/pages/setting/widgets/select_dialog.dart';
import 'package:pilistudy/pages/setting/widgets/slide_dialog.dart';
import 'package:pilistudy/pages/video/reply/widgets/reply_item_grpc.dart';
...
List<SettingsModel> get extraSettings => [
  ...
  getVideoFilterSelectModel(
    ...
    title: '搜索视频时长',
    suffix: 's',
    key: SettingBoxKey.minDurationForSearch,
    values: const [0, 60, 120, 300, 600],
    defaultValue: 0,
  ),
  const SettingsModel(
    settingsType: SettingsType.sw1tch,
  ...
  SettingsModel(
    settingsType: SettingsType.sw1tch,
    title: '全局关闭评论',
    subtitle: '所有视频/番剧均不展示评论区',
    leading: const Icon(Icons.comments_disabled_outlined),
    setKey: SettingBoxKey.disableAllComments,
    defaultVal: false,
    onChanged: (value) {
      if (value) {
        GStorage.setting.put(SettingBoxKey.showVideoReply, false);
        GStorage.setting.put(SettingBoxKey.showBangumiReply, false);
        GStorage.setting.put(SettingBoxKey.defaultShowComment, false);
      }
    },
  ),
  SettingsModel(
    ...
    setKey: SettingBoxKey.disableAllDanmaku,
    defaultVal: false,
    onChanged: (value) {
      if (value) {
        GStorage.setting.put(SettingBoxKey.enableShowDanmaku, false);
      }
    },
  ),
  SettingsModel(
    settingsType: SettingsType.sw1tch,
    title: '护眼提醒',
    subtitle: '连续播放 ${Pref.eyeCareIntervalMinutes}min 后暂停 ${Pref.eyeCareRestSeconds}s',
    leading: const Icon(Icons.remove_red_eye_outlined),
    setKey: SettingBoxKey.enableEyeCare,
    defaultVal: false,
  ),
  SettingsModel(
    ...
    setKey: SettingBoxKey.eyeCareIntervalMinutes,
    getTrailing: () => Text(
      '${Pref.eyeCareIntervalMinutes}min',
      style: Get.theme.textTheme.titleSmall,
    ),
    onTap: (setState) async {
      final choices = [15, 20, 25, 30, 40];
              ...
              child: Column(
                ...
                children: choices.map((c) {
                  return RadioListTile<int>(
                    title: Text('${c} 分钟'),
                    value: c,
                    groupValue: Pref.eyeCareIntervalMinutes,
                    onChanged: (v) {
                      GStorage.setting.put(SettingBoxKey.eyeCareIntervalMinutes, v);
                      Get.back();
                      setState();
                    },
                  );
      ...
      if (result != null) {
        await GStorage.setting.put(SettingBoxKey.eyeCareIntervalMinutes, result);
        setState();
      }
    },
  ),
  SettingsModel(
    ...
    setKey: SettingBoxKey.eyeCareRestSeconds,
    getTrailing: () => Text(
      '${Pref.eyeCareRestSeconds}s',
      style: Get.theme.textTheme.titleSmall,
    ),
    onTap: (setState) async {
      final choices = [10, 20, 30, 60];
      final int? result = await showDialog<int>(
        context: Get.context!,
        builder: (context) {
          return AlertDialog(
            title: const Text('护眼休息时长'),
            content: SingleChildScrollView(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: choices.map((c) {
                  return RadioListTile<int>(
                    title: Text('${c} 秒'),
                    value: c,
                    groupValue: Pref.eyeCareRestSeconds,
                    onChanged: (v) {
                      GStorage.setting.put(SettingBoxKey.eyeCareRestSeconds, v);
                      Get.back();
                      setState();
                    },
                  );
      ...
      if (result != null) {
        await GStorage.setting.put(SettingBoxKey.eyeCareRestSeconds, result);
        setState();
      }
    },
  ),
