...
import 'package:pilistudy/models/home/rcmd/result.dart';
import 'package:pilistudy/models/model_hot_video_item.dart';
import 'package:pilistudy/models/model_rec_video_item.dart';
import 'package:pilistudy/models/pgc_lcf.dart';
import 'package:pilistudy/models/video/play/url.dart';
import 'package:pilistudy/models_new/pgc/pgc_rank/pgc_rank_item_model.dart';
import 'package:pilistudy/models_new/triple/pgc_triple.dart';
...
import 'package:pilistudy/models_new/video/video_detail/video_detail_response.dart';
    ...
    if (res.data['code'] == 0) {
      ...
      for (var i in res.data['data']['items']) {
        ...
        if (i['card_goto'] != 'ad_av' &&
          ...
          // 知识模式 — 仅保留白名单分区
          if (!RecommendFilter.isZoneAllowed(i['args']?['tname'])) {
            continue;
          }
    ...
    if (res.data['code'] == 0) {
      ...
      for (var i in res.data['data']['list']) {
        if (!GlobalData().blackMids.contains(i['owner']['mid']) &&
          ...
          // 知识模式 — 仅保留白名单分区
          if (!RecommendFilter.isZoneAllowed(i['tname'])) {
            continue;
          }
    ...
    if (res.data['code'] == 0) {
      return Success(
        (res.data['result']?['list'] as List?)
            ...
            .where((i) =>
                !RecommendFilter.filterTitle(i.title ?? '', desc: i.desc))
            .toList(),
      );
    } else {
      return Error(res.data['message']);
