...
import 'package:pilistudy/models/user/stat.dart';
import 'package:pilistudy/models_new/coin_log/data.dart';
import 'package:pilistudy/models_new/history/data.dart';
import 'package:pilistudy/models_new/later/data.dart';
import 'package:pilistudy/models_new/login_log/data.dart';
import 'package:pilistudy/models_new/media_list/data.dart';
import 'package:pilistudy/models_new/space_setting/data.dart';
import 'package:pilistudy/models_new/sub/sub/data.dart';
...
class UserHttp {
  ...
  static Future<LoadingState<LaterData>> seeYouLater({
    ...
    if (res.data['code'] == 0) {
      final data = LaterData.fromJson(res.data['data']);
      if (data.list != null) {
        data.list = data.list!
            .where((i) =>
  ...
  static Future<LoadingState<HistoryData>> historyList({
    ...
    if (res.data['code'] == 0) {
      final data = HistoryData.fromJson(res.data['data']);
      if (data.list != null) {
        data.list = data.list!
            .where((i) =>
                !RecommendFilter.isBlacklisted(i.authorMid) &&
