...
import 'package:pilistudy/models/member/info.dart';
import 'package:pilistudy/models/member/tags.dart';
import 'package:pilistudy/models_new/follow/data.dart';
import 'package:pilistudy/models_new/follow/list.dart';
import 'package:pilistudy/models_new/member/coin_like_arc/data.dart';
import 'package:pilistudy/models_new/member/search_archive/data.dart';
import 'package:pilistudy/models_new/member_card_info/data.dart';
import 'package:pilistudy/models_new/space/space/data.dart';
import 'package:pilistudy/models_new/space/space_archive/data.dart';
import 'package:pilistudy/models_new/space/space_article/data.dart';
...
import 'package:pilistudy/models_new/space/space_opus/data.dart';
import 'package:pilistudy/models_new/space/space_season_series/item.dart';
import 'package:pilistudy/models_new/space/space_shop/data.dart';
import 'package:pilistudy/models_new/upower_rank/data.dart';
...
import 'package:pilistudy/utils/wbi_sign.dart';
...
class MemberHttp {
  ...
  static Future<LoadingState<SpaceArchiveData>> spaceArchive({
    ...
    if (res.data['code'] == 0) {
      final data = SpaceArchiveData.fromJson(res.data['data']);
      if (data.item != null) {
        data.item = data.item!
            .where((i) =>
                !RecommendFilter.isBlacklisted(i.owner.mid) &&
  ...
  static Future<LoadingState<SearchArchiveData>> searchArchive({
    ...
    if (res.data['code'] == 0) {
      final data = SearchArchiveData.fromJson(res.data['data']);
      if (data.list?.vlist != null) {
        data.list!.vlist = data.list!.vlist!
            .where((i) => !RecommendFilter.filterAll(i))
            .toList();
      }
      return Success(data);
    } else {
  ...
  static Future<LoadingState<CoinLikeArcData>> coinArc({
    ...
    if (res.data['code'] == 0) {
      final data = CoinLikeArcData.fromJson(res.data['data']);
      if (data.item != null) {
        data.item = data.item!
            .where((i) =>
                !RecommendFilter.filterTitle(i.title ?? ''))
